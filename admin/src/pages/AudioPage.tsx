...
import { Search, Plus, Filter, Music, Play, MoreVertical } from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold text-white font-quote">Аудіо-простір</h1>
      ...
      <div className="bg-df2b-bg-card rounded-xl border border-white/5 p-4">
        ...
        <div className="space-y-2">
          {[1, 2, 3, 4, 5].map((item) => (
            <div key={item} className="flex items-center justify-between p-3 rounded-lg hover:bg-white/5 transition-colors cursor-pointer group">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-lg bg-df2b-accent/10 flex items-center justify-center text-df2b-accent group-hover:bg-df2b-accent group-hover:text-df2b-bg transition-colors">
                  <Play className="w-5 h-5 ml-1" />
                </div>
                <div>
                  <h3 className="font-medium text-white">Вечірній спокій #{item}</h3>
                  <p className="text-sm text-df2b-text-secondary">Медитація • 15:00</p>
                </div>
              </div>
              <div className="flex items-center gap-6">
                <div className="text-sm text-df2b-text-secondary hidden md:block">
                  1,234 прослуховувань
                </div>
